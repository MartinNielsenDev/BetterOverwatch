    ...
    class Program
    {
        ...
        public static void captureDesktop()
        {
            ...
            while (true)
            {
                ...
                {
                    Thread.Sleep(50);
                    customMenu1.currentGame.MenuItems[0].Text = "Game elapsed: " + secondsToMinutes((int)Math.Floor(Convert.ToDouble(Vars.gameTimer.ElapsedMilliseconds / 1000)));
                    ...
                    if (Vars.frameTimer.ElapsedMilliseconds >= Vars.loopDelay)
                    {
                        ...
                        if (frame != null)
                        {
                            try
                            {
                                ...
                                {
                                    if (checkHeroPlayed(frame.DesktopImage) && Vars.roundTimer.ElapsedMilliseconds >= Functions.getTimeDeduction(getNextDeduction: true)) // tracks what heroes you play from the scoreboard (TAB)
                                    {
            ...
            if (!srText.Equals(String.Empty) && srText.Length <= 4) // CHECK SR
            {
                if (Convert.ToInt32(srText) > 1000 && Convert.ToInt32(srText) < 5000)
                {
                    ...
                    if (Vars.srCheck[0].Equals(srText) && Vars.srCheck[1].Equals(srText))
                    {
                        ...
                        {
                            Functions.playSound();
                            customMenu1.currentGame.MenuItems[1].Text = "Skill rating: " + srText;
                        ...
                        if (currentGame >= 2)
                        {
                            ...
                            prepareUploadData();
                            Server.uploadGame(Vars.gameData.GetData(), currentImage);
                            resetGame();
                        }
        ...
        private static void checkCompetitiveGameEntered(Bitmap frame)
        {
            ...
            if (!compText.Equals(String.Empty))
            {
                ...
                if (percent >= 70)
                {
                    ...
                    Vars.gameData.startsr = currentSR;
            ...
            if (Vars.gameData.map.Equals(String.Empty))
            {
                ...
                if (!mapText.Equals(String.Empty))
                {
                    ...
                    if (!mapText.Equals(String.Empty))
                    {
                        ...
                        customMenu1.currentGame.MenuItems[2].Text = "Map: " + mapText;
        ...
        private static void checkTeamsSkillRating(Bitmap frame)
        {
            ...
            {
                customMenu1.currentGame.MenuItems[3].Text = "Team ratings: " + Vars.gameData.team1sr + " | " + Vars.gameData.team2sr;
            }
            else if (!Vars.gameData.team1sr.Equals(String.Empty))
            {
                customMenu1.currentGame.MenuItems[3].Text = "Team ratings: " + Vars.gameData.team1sr + " | -";
            }
            else if (!Vars.gameData.team2sr.Equals(String.Empty))
            {
                customMenu1.currentGame.MenuItems[3].Text = "Team ratings: - | " + Vars.gameData.team2sr;
            }
        }
        ...
        private static bool checkHeroPlayed(Bitmap frame)
        {
                ...
                for (int i = 0; i < Vars.heroNames.Length; i++)
                {
                    ...
                    if (accuracy >= 70)
                    {
                        ...
                        if (currentHero != i)
                        {
                            ...
                            currentHero = i;
                            customMenu1.currentGame.MenuItems[4].Text = "Last Hero: " + Vars.heroNamesReal[i];
                            break;
                        }
                    }
                }
            ...
            if (Vars.gameData.team1score.Equals(String.Empty) && Vars.gameData.team1score.Equals(String.Empty))
            {
                ...
                if (!scoreTextLeft.Equals(String.Empty) && !scoreTextRight.Equals(String.Empty))
                {
                    ...
                    if (team1 >= 0 && team1 <= 6 && team2 >= 0 && team2 <= 6)
                    {
                        ...
                        customMenu1.currentGame.MenuItems[5].Text = "Final score: " + scoreTextLeft + " | " + scoreTextRight;
