...
namespace BetterOverwatch.Forms
{
    public partial class UpdateNotificationForm : Form
    {
        ...
        private void DownloadUpdate()
        {
            using (WebClient webClient = new WebClient())
            {
                try
                {
                    ...
                    webClient.DownloadFileAsync(new Uri(downloadUrl), Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"overwatchtracker\betteroverwatch.exe"));
                }
                catch { }
            }
        }
        ...
        {
            string argument = "/C choice /C Y /N /D Y /T 4 & Del /F /Q \"{0}\" & choice /C Y /N /D Y /T 4 & Move /Y \"{1}\" \"{2}\" & Start \"\" /D \"{3}\" \"{4}\" {5}";
            string tempPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"overwatchtracker\betteroverwatch.exe");
